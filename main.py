...
async def get_transcription(
    url: str = Query(...),
    lang: str = Query("en"),
    format: str = Query("text"),  # text, json, srt
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if not available_subs:
                return JSONResponse(
                    status_code=404,
                    content={"error": f"No subtitles found for language '{lang}'", 
            ...
            if format == "json":
                ...
                # For text format, we return the subtitle URL since actual text extraction
                # would require downloading and parsing the subtitle file
                return JSONResponse(content={
                    "message": "Text extraction requires downloading subtitle file",
                    "subtitle_url": subtitle_info.get('url'),
                    "instructions": "Download the subtitle file from the URL and parse it to extract plain text"
                })
